...
hide_window_config = False  # Flow/VEO renders more reliably when the token window is visible.
...
def _is_recaptcha_reload(url: str) -> bool:
    value = str(url or "")
    return "/recaptcha/" in value and "/reload" in value
...
def _extract_recaptcha_token(text: str):
    ...
    match = re.search(r'"rresp"\s*,\s*"([^"]+)"', str(text or ""))
    if match:
        return match.group(1)
    match = re.search(r'"uvresp"\s*,\s*"([^"]+)"', str(text or ""))
    if match:
        return match.group(1)
    return None
...
class TokenCollector:
    ...
    async def _start_browser(self):
        ...
        try:
            ...
            await self._prepare_warmup_and_token_page()
        except Exception as e:
    ...
    async def _prepare_warmup_and_token_page(self):
        """Keep one normal Flow tab alive, then use a separate routed tab for token capture."""
        if not self.context:
            return
        ...
        self._warmup_page = pages[-1] if pages else await self.context.new_page()
        try:
            if not self._is_project_url(self._warmup_page.url or ""):
                await self._warmup_page.goto(self.project_url, wait_until="domcontentloaded", timeout=25000)
            await self._warmup_page.wait_for_timeout(1500)
            self._log("✅ Da mo tab VEO3 warm-up rieng, khong chan request UI")
        except Exception as exc:
            self._debug_log(f"⚠️ Warm-up tab VEO3 chua san sang: {exc}")

        self.page = await self.context.new_page()
        try:
            await self.page.goto(self.project_url, wait_until="domcontentloaded", timeout=25000)
        except Exception as exc:
            self._debug_log(f"⚠️ Token tab goto VEO3 loi: {exc}")
        await self._show_warmup_page()

    async def _show_warmup_page(self):
        try:
            if self._warmup_page and not self._warmup_page.is_closed():
                await self._warmup_page.bring_to_front()
        except Exception:
            pass
    ...
    async def _wait_web_stable_by_create_button(self, timeout_ms: int = 30000) -> bool:
        ...
        while time.time() < deadline:
            if self._should_stop():
                return False
            if self.mode == "generate_image":
                if await self._is_agent_composer_ready():
                    self._log("✅ Web Stabled")
                    return True
            else:
                btn = await self._find_create_button(timeout_ms=300)
                if btn is not None:
                    self._log("✅ Web Stabled")
                    return True
    ...
    async def _is_agent_composer_ready(self) -> bool:
        if not self.page:
            return False
        try:
...
() => {
  ...
  const isPromptInput = (el) => {
    const label = labelOf(el);
    return visible(el) && (
      el.id === 'PINHOLE_TEXT_AREA_ELEMENT_ID' ||
      label.includes('bạn muốn tạo') ||
      label.includes('ban muon tao') ||
...
"""
    ...
    async def _find_create_button(self, timeout_ms: int = 2000):
        """Find the Flow create/submit button across locale and small DOM changes."""
        if not self.page:
            return None

        if self.mode == "generate_image":
            handle = await self._find_agent_composer_arrow(timeout_ms=timeout_ms)
        ...
        selectors = [
            "button:has-text('Tạo'):has(i:has-text('arrow_forward'))",
        ...
        while time.time() < deadline:
            for selector in selectors:
                try:
                    ...
                    accepts_icon_only = "arrow_forward" in selector or "send" in selector
                    count = await locator.count()
                    for idx in range(count):
                        candidate = locator.nth(idx)
                        try:
                            ...
                            if any(blocked in haystack for blocked in excluded_texts):
                                continue
                            box = None
                            try:
                                ...
                                box = None
                            panel_send_button = bool(
                                box
                                and float(box.get("x", 0)) > 900
                                and float(box.get("y", 0)) > 650
                                and float(box.get("width", 0)) <= 90
                                and float(box.get("height", 0)) <= 90
                            )
                            if not (
                                accepts_icon_only
                                or panel_send_button
                                or "Tạo" in haystack
                                or "Create" in haystack
                                or "Send" in haystack
                                or "Gửi" in haystack
                                or "arrow_forward" in haystack
                                or "send" in haystack
                            ):
                                continue
                            return candidate
    ...
    async def _find_agent_composer_arrow(self, timeout_ms: int = 2000):
        """Find only the arrow submit button inside Flow Agent composer."""
        if not self.page:
            return None
        deadline = time.time() + (max(200, timeout_ms) / 1000.0)
        while time.time() < deadline:
...
() => {
  ...
  const inputs = Array.from(document.querySelectorAll("textarea,div[contenteditable='true'],[role='textbox']"))
    .filter(visible)
    .map((el) => ({el, rect: el.getBoundingClientRect(), label: textOf(el)}))
    .filter((item) => {
      const r = item.rect;
      return r.left > window.innerWidth * 0.55 &&
        r.top > window.innerHeight * 0.60 &&
        r.bottom < window.innerHeight - 8 &&
        r.width > 180 &&
        (item.label.includes('bạn muốn tạo') ||
         item.label.includes('ban muon tao') ||
         item.label.includes('what do you want') ||
         item.el.tagName === 'TEXTAREA' ||
         item.el.getAttribute('contenteditable') === 'true');
    })
    .sort((a, b) => b.rect.bottom - a.rect.bottom);
  if (!inputs.length) return null;
  const inputRect = inputs[0].rect;
  const composerLeft = Math.max(window.innerWidth * 0.55, inputRect.left - 40);
  const composerTop = Math.max(window.innerHeight * 0.55, inputRect.top - 80);
  const composerRight = Math.min(window.innerWidth - 4, inputRect.right + 90);
  const composerBottom = Math.min(window.innerHeight - 4, inputRect.bottom + 90);
  const buttons = Array.from(document.querySelectorAll('button,[role="button"]'))
    .filter(visible)
    .map((el) => ({el, rect: el.getBoundingClientRect(), label: textOf(el)}))
    .filter((item) => {
      const r = item.rect;
      const inComposer = r.left >= composerLeft && r.right <= composerRight &&
        r.top >= composerTop && r.bottom <= composerBottom;
      const nearRight = r.left > inputRect.right - 95 && r.right <= composerRight;
      const nearBottom = r.top >= inputRect.top - 20 && r.bottom <= composerBottom;
      const compact = r.width >= 28 && r.width <= 72 && r.height >= 28 && r.height <= 72;
      const submitLike = item.label.includes('send') ||
        item.label.includes('gửi') ||
        item.label.includes('gui') ||
        item.label.includes('arrow') ||
        item.el.querySelector("svg,path,i,mat-icon") !== null;
      const disabled = item.el.disabled ||
        item.el.getAttribute('aria-disabled') === 'true' ||
        item.el.className.toString().toLowerCase().includes('disabled');
      return inComposer && nearRight && nearBottom && compact && submitLike && !disabled;
    })
    .sort((a, b) => {
      const ad = Math.abs(a.rect.right - composerRight) + Math.abs(a.rect.bottom - composerBottom);
      const bd = Math.abs(b.rect.right - composerRight) + Math.abs(b.rect.bottom - composerBottom);
      return ad - bd;
    });
  return buttons.length ? buttons[0].el : null;
}
"""
                )
    ...
    async def _find_agent_composer_arrow(self, timeout_ms: int = 2000):
        """DOM-only submit lookup: input placeholder -> composer parent -> send arrow."""
        if not self.page:
            return None
        deadline = time.time() + (max(200, timeout_ms) / 1000.0)
        while time.time() < deadline:
...
() => {
  ...
  const input = document.querySelector("[data-codex-flow-composer-input='1']") ||
  ...
  let composer = input;
  for (let i = 0; i < 10 && composer && composer.parentElement; i += 1) {
    composer = composer.parentElement;
    const buttons = Array.from(composer.querySelectorAll('button,[role="button"]')).filter(visible);
  ...
  const buttons = Array.from(composer.querySelectorAll('button,[role="button"]'))
    ...
    .filter((el) => {
      ...
      const blocked = label.includes('add') || label.includes('thêm') ||
        label.includes('them') || label.includes('upload') || label.includes('tải') ||
        label.includes('tai') || label.includes('settings') || label.includes('cài đặt') ||
        label.includes('cai dat') || label.includes('sliders');
      const submitLike = label.includes('send') || label.includes('gửi') ||
        label.includes('gui') || label.includes('arrow') ||
        el.querySelector("svg,path,i,mat-icon") !== null;
      return submitLike && !blocked && !disabled;
    });
...
"""
    ...
    async def _prepare_flow_prompt_ui(self, disable_agent: bool = True) -> None:
        """Prepare Flow's prompt UI. In image mode we keep the Agent panel enabled."""
        if not self.page:
            return
        script = r"""
(disableAgent) => {
  ...
  const hasVisiblePromptInput = () => Array.from(document.querySelectorAll(
      ...
      return rect.bottom > window.innerHeight * 0.45 &&
        (rect.left > window.innerWidth * 0.45 || rect.width > 300 ||
          label.includes('bạn muốn tạo') || label.includes('ban muon tao') ||
          label.includes('what do you want'));
    });
  ...
  if (disableAgent) {
    const closeCandidates = Array.from(document.querySelectorAll('button,[role="button"]'))
      ...
      .filter((el) => {
        const rect = el.getBoundingClientRect();
        const label = textOf(el).toLowerCase();
        const rightPanel = rect.left > window.innerWidth * 0.58;
        const iconOnlyPanelClose = rightPanel && rect.top > 175 &&
          rect.right > window.innerWidth * 0.88 && rect.width <= 56 && rect.height <= 56;
        return rightPanel && (
          label === 'x' || label === '×' || label.includes('close') ||
          label.includes('dismiss') || label.includes('đóng') ||
          label.includes('dong') || iconOnlyPanelClose
        );
      })
  ...
  } else if (false && !hasVisiblePromptInput()) {
    const agentPattern = /(tác nhân|tac nhan|agent)/i;
    const agentCandidates = Array.from(document.querySelectorAll('button,[role="button"],div,span'))
      .filter(visible)
      .filter((el) => agentPattern.test(textOf(el)))
      .map((el) => el.closest('button,[role="button"]') || el)
      .filter((el, index, arr) => arr.indexOf(el) === index)
      .map((el) => ({el, rect: el.getBoundingClientRect()}))
      .filter((item) => item.rect.top > window.innerHeight * 0.35 &&
        item.rect.width <= 280 && item.rect.height <= 110)
      .sort((a, b) => Math.abs(a.rect.bottom - window.innerHeight) - Math.abs(b.rect.bottom - window.innerHeight));
    for (const item of agentCandidates) {
      if (clickElement(item.el)) {
        clickedAgent = true;
        break;
      }
    }
  ...
  return {closedPanel, clickedAgent};
}
"""
        try:
            result = await self.page.evaluate(script, bool(disable_agent))
            if isinstance(result, dict):
                if result.get("closedPanel"):
                    self._log("✅ Đã đóng panel Flow bên phải")
                if result.get("clickedAgent"):
                    self._log("✅ Đã mở/giữ Tác nhân để nhập prompt")
            if disable_agent:
                await self.page.wait_for_timeout(500)
                agent_box = await self.page.evaluate(
                    r"""
() => {
  const visible = (el) => {
    if (!el || !(el instanceof HTMLElement)) return false;
  ...
  const candidates = Array.from(document.querySelectorAll('button,[role="button"],div,span'))
    ...
    .map((el) => ({el, rect: el.getBoundingClientRect()}))
    .filter((item) => item.rect.top > window.innerHeight * 0.35 &&
      item.rect.width <= 260 && item.rect.height <= 100)
    .sort((a, b) => Math.abs(a.rect.bottom - window.innerHeight) - Math.abs(b.rect.bottom - window.innerHeight));
...
"""
                ...
                if isinstance(agent_box, dict):
                    await self.page.mouse.move(float(agent_box["x"]), float(agent_box["y"]))
                    await self.page.wait_for_timeout(100)
                    await self.page.mouse.click(float(agent_box["x"]), float(agent_box["y"]))
                    self._log("✅ Đã click chuột thật vào nút Tác nhân")
                    await self.page.wait_for_timeout(600)
            await self.page.wait_for_timeout(350)
        except Exception as e:
            self._debug_log(f"Could not prepare Flow prompt UI: {e}")
...
(text) => {
  ...
  const candidates = Array.from(document.querySelectorAll(
    ...
    .map((el) => {
      ...
      const label = textOf(el);
      let score = 0;
      const inRightAgentComposer = rect.left > window.innerWidth * 0.55 &&
        rect.top > window.innerHeight * 0.68 &&
        rect.bottom > window.innerHeight * 0.78 &&
        rect.bottom < window.innerHeight - 8 &&
        rect.width > 180;
      if (!inRightAgentComposer) score -= 1000;
      if (label.includes('bạn muốn tạo') || label.includes('ban muon tao')) score += 120;
      if (label.includes('what do you want')) score += 120;
      if (label.includes('prompt') || label.includes('textbox')) score += 30;
      if (rect.left > window.innerWidth * 0.55) score += 80;
      if (rect.top > window.innerHeight * 0.55) score += 60;
      if (rect.width > 250) score += 20;
...
""",
            ...
            if isinstance(result, dict):
                self._log(
                    f"✅ Đã nạp đúng prompt vào Tác nhân VEO3 "
                    f"({result.get('tag')}, x={result.get('x')}, y={result.get('y')})"
                )
                return True
        except Exception:
            pass
        ...
        for selector, input_type in candidates:
            try:
                ...
                self._log(f"✅ Đã nạp prompt vào VEO3 bằng selector: {selector}")
                return True
            except Exception:
                continue
    ...
    async def _fill_prompt_input_text(self, text: str) -> bool:
        """Fill only the real Flow Agent composer input found by placeholder/aria-label."""
        if not self.page:
            return False
...
(text) => {
  ...
  el.focus();
  el.setAttribute('data-codex-flow-composer-input', '1');
  if (el.tagName === 'TEXTAREA' || el.tagName === 'INPUT') {
    const proto = el.tagName === 'TEXTAREA' ? HTMLTextAreaElement.prototype : HTMLInputElement.prototype;
    const setter = Object.getOwnPropertyDescriptor(proto, 'value')?.set;
    if (setter) setter.call(el, text);
...
""",
            ...
            if isinstance(result, dict):
                self._log(
                    "✅ Đã nạp prompt vào đúng ô nhập Tác nhân VEO3 "
                    f"({result.get('tag')}, placeholder='{result.get('placeholder') or result.get('aria')}')"
                )
                return True
        except Exception as exc:
            self._debug_log(f"Không nạp được prompt bằng DOM composer: {exc}")

        return False

    def _token_prompt_text(self, prompt_text=None) -> str:
        text = str(prompt_text or "").strip()
        if not text:
            return "Create one image only."
        text = re.sub(r"[\x00-\x08\x0b\x0c\x0e-\x1f]+", " ", text)
        text = re.sub(r"\s+", " ", text).strip()
        return text[:1200].rstrip() or "Create one image only."
    ...
    async def _on_response(self, response):
        ...
        if token_value and self._token_future and not self._token_future.done():
            self._log("✅ TokenCollector: đã bắt được recaptcha token từ browser")
    ...
    async def _trigger_token(self, clear_storage=False, prompt_text=None):
        ...
        for attempt in range(1, self._trigger_retry_max + 1):
            ...
            if not self._is_project_url(self.project_url):
                try:
                    ...
                    return False
            ok = await self._trigger_token_once(clear_storage=clear_storage, prompt_text=prompt_text)
            if ok:
                return True
            if self._login_required:
                return False
    ...
    async def _trigger_token_once(self, clear_storage=False, _has_reloaded=False, prompt_text=None):
        if self._should_stop():
        ...
        if self.mode == "generate_image":
            if detected_mode == "generate_image":
                ...
                image_mode_ok = True
                self._image_mode_ready = True
                self._log("ℹ️ [generate_image] Flow UI mới không còn nút mode cũ; bỏ qua bước chọn mode")
            else:
            ...
            if image_mode_ok:
                ...
                if not ok:
                    self._log("⚠️ [generate_image] Không chọn được mode cũ; tiếp tục theo Flow UI mới")
                    self._image_mode_ready = True
        else:
        ...
        try:
            ...
            await self._prepare_flow_prompt_ui(disable_agent=False)
            ...
            while time.time() - timeout_start < textarea_timeout:
                if self._should_stop():
                    ...
                    return False
                try:
                    ok = await self._fill_prompt_input_text(self._token_prompt_text(prompt_text))
                    if ok:
            ...
            try:
                ...
                while time.time() - timeout_start < 10:
                    if self._should_stop():
                        ...
                        return False
                    try:
                        create_button = await self._find_create_button(timeout_ms=500)
                        if create_button is not None:
                            clicked = await self._click_with_fallback(create_button, label="create button")
                            if clicked:
                                await self._show_warmup_page()
                                return True
                    except Exception:
        ...
        if not _has_reloaded:
            ...
            if await self._reload_project_page():
                return await self._trigger_token_once(clear_storage=clear_storage, _has_reloaded=True, prompt_text=prompt_text)

        self._log(f"❌ Không Lấy được token sau {max_button_retries} lần thử")
        return False

    async def get_token(self, clear_storage=False, token_timeout_override=None, prompt_text=None):
        if self._should_stop():
        ...
        try:
            ...
            try:
                ok = await self._trigger_token(clear_storage=clear_storage, prompt_text=prompt_text)
                if not ok:
                    if self._login_required:
                        return None
            ...
            try:
                ...
                if self.mode == "generate_image":
                    self._log("⚠️ Flow UI mới không trả recaptcha token; thử gửi request tạo ảnh bằng token fallback")
                    return "FLOW_UI_NEW_TOKEN_FALLBACK"
                return None
            except Exception:
