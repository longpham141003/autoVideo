...
class TextToVideoWorkflow(QThread):
	...
	def __init__(self, project_name=None, project_data=None, parent=None):
		...
		self._download_images = bool(self.project_data.get("download_images", False))
	...
	def _save_response_json(self, response, prompt_id, prompt_text, flow="text_to_video"):
		"""Lưu response tạo video để debug lỗi Google Flow/AI Sandbox."""
		try:
			project_dir = WORKFLOWS_DIR / str(self.project_name)
			project_dir.mkdir(parents=True, exist_ok=True)
			...
			self._log(f"⚠️ Không thể lưu response.json: {e}")
				...
				for idx_prompt, prompt in enumerate(prompts):
					...
					try:
						...
						while True:
							...
							for attempt in range(max_token_retries):
								...
								try:
									...
									token = await asyncio.wait_for(
										collector.get_token(clear_storage=clear_storage, prompt_text=prompt_text),
							...
							self._save_response_json(response, prompt_id, prompt_text, flow="text_to_video")
							...
							retryable_errors = {"401", "403", "408", "409", "429", "500", "502", "503", "504", "3", "13", "53"}
							...
							if not response.get("ok", True) and error_code_str in retryable_errors:
								...
								if error_code_str == "403" and consecutive_403_count == 2:
									...
									try:
										await asyncio.wait_for(
											collector.get_token(clear_storage=True, prompt_text=prompt_text),
											timeout=60
										)
										prompt_retry_counts[f"{prompt_id}_403_count"] = 0
								...
								if error_code_str in {"401", "429", "500", "502", "503", "504"}:
									self._log(f"⚠️ Lỗi {error_code_str}, restart Chrome rồi retry prompt {prompt_id}")
									await collector.restart_browser()
							...
							if not operations:
								msg = error_message or response.get("reason") or response.get("error") or str(response_body or "")[:500] or "No operations returned"
								if retry_count < retry_with_error:
									retry_count += 1
									prompt_retry_counts[prompt_id] = retry_count
									self._discard_scene_ids(prompt_id, scene_ids)
									self._log(
										f"⚠️ API chưa nhận operation: {msg[:140]}. "
										f"Chờ {wait_resend}s rồi retry prompt {prompt_id} ({retry_count}/{retry_with_error})"
									)
									if not await self._sleep_with_stop(wait_resend):
										break
									continue

								self._log(f"❌ API lỗi, không tạo được video: {msg[:160]}")
								for idx, scene_id in enumerate(scene_ids):
									self._update_state_entry(
										...
										idx,
										"FAILED",
										error=error_code_str or error_code,
										message=msg,
									)
									self.video_updated.emit({
										"prompt_idx": f"{prompt_id}_{idx + 1}",
			...
			async with collector:
				for prompt_id, prompt_text, scene_id, idx in resend_items:
					...
					for resend_attempt in range(retry_with_error):
						...
						self._save_response_json(response, prompt_id, prompt_text, flow="text_to_video_resend")
	...
	def _handle_status_response(self, response_body):
		...
		for op in operations:
			...
			if status == "MEDIA_GENERATION_STATUS_SUCCESSFUL":
				...
				if self._download_images and image_url:
