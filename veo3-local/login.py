...
async def login_veo3_auto(username, password, profile_name=None, log_callback=None, stop_check=None):
    ...
    async def _switch_to_google_signin_page(context_obj, current_page):
        for candidate in reversed(context_obj.pages):
            try:
                url = str(candidate.url or "")
                if "accounts.google.com" in url:
                    await candidate.bring_to_front()
                    return candidate
                email_box = candidate.locator('input[type="email"]').first
                if await email_box.is_visible(timeout=1000):
                    await candidate.bring_to_front()
                    return candidate
            except Exception:
                continue
    ...
    async def _find_create_flow_button(page_obj):
        selectors = [
            "button:has-text('Create with Google Flow')",
            "button:has-text('Create with Flow')",
            "button:has-text('Tao bang Google Flow')",
            "button:has-text('Tao bang Flow')",
            "xpath=//button[contains(normalize-space(.), 'Create with Google Flow')]",
            "xpath=//button[contains(normalize-space(.), 'Create with Flow')]",
            "xpath=//button[contains(normalize-space(.), 'Google Flow')]",
        ]
    ...
    async def _detect_and_save_account_type():
        ...
        async def _has_visible_exact_text(text):
            try:
                locator = page.locator(f"xpath=//*[normalize-space()='{text}']")
        ...
        try:
            if await _has_visible_exact_text("ULTRA"):
                account_type = "ULTRA"
            elif await _has_visible_exact_text("PRO"):
                account_type = "PRO"
        except Exception:
            pass
    ...
    try:
        ...
        for attempt in range(4):
            ...
            for check in range(4):
                if _should_stop():
                    return {"success": False, "stopped": True, "already_logged_in": False, "message": "Đã dừng auto login."}
                try:
                    btn = await _find_create_flow_button(page)
                    if btn is not None:
                        found_create_btn = True
                        await _click_with_delay(btn)
                        page = await _switch_to_google_signin_page(context, page)
        ...
        for _ in range(3):
            ...
            if next_btn:
                ...
                await _wait_for_ready(page, 'input[type="password"]', timeout=15000)
                break
            await asyncio.sleep(1)
        if not next_btn:
